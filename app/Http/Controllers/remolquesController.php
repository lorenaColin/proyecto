...
class remolquesController extends Controller
{
    ...
    public function store(remolqueRequest $request)
    {
        ...
        $nuevoRemolque = remolques::create([  'SubTipoRem' => $remolque->Clave,   
           'placa' => $request->placa,    'uuid_company' => $request->uuid_company,  
        ]);
    ...
    public function show($id)
    {
        ...
        if (!$remolques) {
            return ApiResponse::error('remolques no encontrada', 404, 'La remolques 
    ...
    public function update(remolqueRequest $request, $id)
    {
        try {
            ...
            if (!$remolque) {
                return ApiResponse::error('remolque no encontrada', 404, 'El remolque
                 con el ID proporcionado no existe');
            }
